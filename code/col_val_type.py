...
keyword = sys.argv[1]
query = '''SELECT metadata.file_name as path,meta.name from metadata right join meta on metadata.file_name=meta.file_path where lower(col_name) like '%'''+col_name+'''%' and lower(word) like '%'''+col_val+'''%' '''
